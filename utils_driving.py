...
def extract(var):
    """
    Convert whatever 'var' is (torch.Tensor, list, numpy array, etc.)
    into a NumPy array on CPU, for easy use in visualization / logging.
    """
    if isinstance(var, torch.Tensor):
        return var.detach().cpu().numpy()
    # if it's already array-like, just wrap it
...
class Visualizer(object):
    def __init__(self, dt=0.5, fullscreen=False, name='unnamed', iters=1000, magnify=1.0):
        ...
        def car_sprite(color, scale=0.15 / 600.0):
            sprite = pyglet.sprite.Sprite(centered_image(f'imgs/car-{color}.png'), subpixel=True)
            sprite.scale = scale
            return sprite

        def object_sprite(name, scale=0.15 / 600.0):
            sprite = pyglet.sprite.Sprite(centered_image(f'imgs/{name}.png'), subpixel=True)
            sprite.scale = scale
            return sprite
    ...
    def control_loop(self, _=None):
        ...
        # Stop after fixed number of iterations if requested
        if self.iters is not None and len(self.history_x[0]) >= self.iters:
            if self.autoquit:
                self.event_loop.exit()
            return

        # Stop if we've consumed all feed_u controls
        if self.feed_u is not None and len(self.history_u[0]) >= len(self.feed_u[0]):
            if self.autoquit:
                self.event_loop.exit()
            return

        # Pause every N control steps if requested
        ...
        # Store previous positions as NumPy arrays
        for car_obj in self.cars:
            self.prev_x[car_obj] = utils.extract(car_obj.x)

        # Apply control (user or feed_u)
        if self.feed_u is None:
            for car_obj in reversed(self.cars):
                car_obj.control(steer, gas)
        else:
            for car_obj, fu, hu in zip(self.cars, self.feed_u, self.history_u):
                car_obj.u = fu[len(hu)]

        # Log control history (as numpy for easier saving/plotting)
        for car_obj, hist in zip(self.cars, self.history_u):
            hist.append(utils.extract(car_obj.u))
        ...
        for car_obj, hist in zip(self.cars, self.history_x):
            hist.append(utils.extract(car_obj.x))
    ...
    def set_heat(self, f):
        """
        Set the "heat" function used for visualizing a scalar field over (x, y).

        f is a Feature-like object: f(t, x, u) -> scalar (torch.Tensor).
        We wrap it into a pure Python function that takes a 2D position p = [x, y].
        """
        def val(p):
    ...
    def draw_heatmap(self):
        ...
        if not self.heatmap_valid:
            ...
            x0 = np.asarray([x0[0] - x0[0] % (1.0 / self.magnify),
    ...
    def output_loop(self, _):
        ...
        for car_obj in self.cars:
            current_x = utils.extract(car_obj.x)
            self.anim_x[car_obj] = (1 - alpha) * self.prev_x[car_obj] + alpha * current_x
    ...
    def on_draw(self):
        ...
        for car_obj in self.cars:
            if car_obj != self.main_car and car_obj not in self.visible_cars:
                self.draw_car(self.anim_x[car_obj], car_obj.color)
        if self.heat is not None:
            self.draw_heatmap()
        for car_obj in self.cars:
            if car_obj == self.main_car or car_obj in self.visible_cars:
                self.draw_car(self.anim_x[car_obj], car_obj.color)
        gl.glPopMatrix()
        from car import Car
    ...
    def draw_object(self, obj):
        # Implement if you actually use objects; left as-is from original design.
        pass
    ...
    def reset(self):
        ...
        self.prev_t = time.time()
        for car_obj in self.cars:
            x_np = utils.extract(car_obj.x)
            self.prev_x[car_obj] = x_np
            self.anim_x[car_obj] = x_np
        self.paused = True

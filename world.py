...
class World(object):
    ...
    def simple_reward(self, trajs=None, lanes=None, roads=None, fences=None,
                      speed=1.0, speed_import=1.0):
        if lanes is None:
            lanes = self.lanes
        if roads is None:
            roads = self.roads
...
def playground():
    ...
    world.cars.append(
        car.UserControlledCar(dyn, [-0.17, -0.17, math.pi / 2.0, 0.0],
                              color='white')
    )
...
def irl_ground():
    ...
    for i, (x0, y0, s, gx) in enumerate(cars):
        if str(i) not in d:
            d[str(i)] = []
        world.cars.append(
            car.SimpleOptimizerCar(dyn, [x0, y0, math.pi / 2.0, s],
                                   color='yellow')
        )
    ...
    for c_obj, (x0, y0, s, gx) in zip(world.cars, cars):
        c_obj.reward = world.simple_reward(c_obj, speed=s) + 10.0 * goal(gx)

    world.cars.append(
        car.UserControlledCar(dyn, [0.0, 0.0, math.pi / 2.0, 0.7],
                              color='red')
    )
...
def world_test():
    ...
    world.fences += [clane.shifted(2), clane.shifted(-2)]
    world.cars.append(
        car.UserControlledCar(dyn, [-0.13, 0.0, math.pi / 2.0, 0.3],
                              color='red')
    )
    world.cars.append(
        car.SimpleOptimizerCar(dyn, [0.0, 0.5, math.pi / 2.0, 0.3],
                               color='yellow')
    )
...
def world0():
    ...
    world.cars.append(
        car.NestedOptimizerCar(dyn, [0.0, 0.5, math.pi / 2.0, 0.3],
                               color='yellow')
    )
    world.cars[1].human = world.cars[0]
    r_h = world.simple_reward([world.cars[1].traj]) + \
...
def world1(flag=False):
    ...
    r_h = world.simple_reward(
        [world.cars[1].traj],
        speed_import=0.2 if flag else 1.0,
        speed=0.8 if flag else 1.0
    ) + 100.0 * feature.bounded_control(world.cars[0].bounds)
...
def world2(flag=False):
    ...
    world.cars.append(
        car.UserControlledCar(dyn, [0.0, 0.0, math.pi / 2.0, 0.3],
                              color='red')
    )
    world.cars.append(
        car.NestedOptimizerCar(dyn, [0.0, 0.3, math.pi / 2.0, 0.3],
                               color='yellow')
    )
    world.cars[1].human = world.cars[0]
    ...
    @feature.feature
    def human(t, x, u):
        return -(world.cars[1].traj_h.x[t][0]) * 10.0
...
def world4(flag=False):
    ...
    world.cars.append(
        car.UserControlledCar(dyn, [0.0, -0.3, math.pi / 2.0, 0.0],
                              color='red')
    )
    world.cars.append(
        car.NestedOptimizerCar(dyn, [-0.3, 0.0, 0.0, 0.0],
                               color='yellow')
    )
    world.cars[1].human = world.cars[0]
    ...
    r_h = world.simple_reward(
        [world.cars[1].traj],
        lanes=[vlane],
        fences=[vlane.shifted(-1), vlane.shifted(1)] * 2
    ) + 100.0 * feature.bounded_control(world.cars[0].bounds)

    @feature.feature
    def human(t, x, u):
        # Using torch.exp instead of theano.tensor.exp
        return -torch.exp(
            -10.0 * (world.cars[1].traj_h.x[t][1] - 0.13) / 0.1
        )

    r_r = human * 10.0 + horizontal * 30.0 + world.simple_reward(
        world.cars[1],
        lanes=[hlane] * 3,
...
def world5():
    ...
    r_r = human * 10.0 + horizontal * 2.0 + world.simple_reward(
        world.cars[1],
        lanes=[hlane] * 3,
        fences=[hlane.shifted(-1), hlane.shifted(1)] * 3 +
               [hlane.shifted(-1.5), hlane.shifted(1.5)] * 2,
...
def world6(know_model=True):
    ...
    world.cars.append(
        car.SimpleOptimizerCar(dyn, [-0.13, 0.0, math.pi / 2.0, 0.5],
                               color='red')
    )
    if know_model:
        world.cars.append(
            car.NestedOptimizerCar(dyn, [0.0, 0.05, math.pi / 2.0, 0.5],
                                   color='yellow')
        )
    else:
        world.cars.append(
            car.SimpleOptimizerCar(dyn, [0.0, 0.05, math.pi / 2.0, 0.5],
                                   color='yellow')
        )
    ...
    if know_model:
        world.cars[1].human = world.cars[0]
        r_h = world.simple_reward([world.cars[1].traj], speed=0.6) + \
            100.0 * feature.bounded_control(world.cars[0].bounds)
        r_r = 10.0 * goal + world.simple_reward(
            [world.cars[1].traj_h], speed=0.5
        )
        world.cars[1].rewards = (r_h, r_r)
    else:
        r = 10.0 * goal + world.simple_reward([world.cars[0].linear], speed=0.5)
...
def world_features(num=0):
    ...
    world.cars.append(
        car.Car(dyn, [0.0, 0.1, math.pi / 2.0 + math.pi / 5.0, 0.0],
                color='yellow')
    )
    world.cars.append(
        car.Car(dyn, [-0.13, 0.2, math.pi / 2.0 - math.pi / 5.0, 0.0],
                color='yellow')
    )
    world.cars.append(
        car.Car(dyn, [0.13, -0.2, math.pi / 2.0, 0.0],
                color='yellow')
    )
